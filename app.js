...
const showError = m => { const box=$('error-box'); if(!box) return; box.classList.remove('hidden'); box.textContent+=`[${new Date().toLocaleTimeString()}] ${m}\n`; box.scrollTop=box.scrollHeight; console.error(m); };
const newId = () => (crypto?.randomUUID?.() || String(Date.now()));
const isFiniteNum = x => Number.isFinite(x);
function toLatLon(o){ if(!o) return null; const lat=Number(o.lat); const lon=Number(o.lon??o.lng); return (isFiniteNum(lat)&&isFiniteNum(lon))?{lat,lon}:null; }
function isValidLatLon(lat,lon){ return isFiniteNum(lat)&&isFiniteNum(lon)&&Math.abs(lat)<=90&&Math.abs(lon)<=180&&!(lat===0&&lon===0); }

/* ===== Géodésie & SEC ===== */
function haversineMeters(a,b){ const R=6371000,toRad=d=>d*Math.PI/180; const dLat=toRad(b.lat-a.lat),dLon=toRad(b.lon-a.lon); const lat1=toRad(a.lat),lat2=toRad(b.lat); const h=Math.sin(dLat/2)**2+Math.cos(lat1)*Math.cos(lat2)*Math.sin(dLon/2)**2; return 2*R*Math.asin(Math.sqrt(h)); }
const distKm=(a,b)=>haversineMeters(a,b)/1000;
function circleFrom2(a,b){ return { c:{lat:(a.lat+b.lat)/2, lon:(a.lon+b.lon)/2 }, rKm:distKm(a,b)/2 }; }
function circleFrom3(a,b,c){ const ax=a.lon,ay=a.lat,bx=b.lon,by=b.lat,cx=c.lon,cy=c.lat; const d=2*(ax*(by-cy)+bx*(cy-ay)+cx*(ay-by)); if(Math.abs(d)<1e-12) return null; const ux=((ax*ax+ay*ay)*(by-cy)+(bx*bx+by*by)*(cy-ay)+(cx*cx+cy*cy)*(ay-by))/d; const uy=((ax*ax+ay*ay)*(cx-bx)+(bx*bx+by*by)*(ax-cx)+(cx*cx+cy*cy)*(bx-ax))/d; const center={lon:ux,lat:uy}; const r=Math.max(distKm(center,a),distKm(center,b),distKm(center,c)); return { c:center, rKm:r }; }
function isIn(c,p){ return distKm(c.c,p)<=c.rKm+1e-6; }
function shuffle(a){ for(let i=a.length-1;i>0;i--){ const j=(Math.random()*(i+1))|0; [a[i],a[j]]=[a[j],a[i]]; } return a; }
function secWelzl(P,R=[]){ if(P.length===0||R.length===3){ if(R.length===0) return null; if(R.length===1) return {c:R[0],rKm:0}; if(R.length===2) return circleFrom2(R[0],R[1]); return circleFrom3(R[0],R[1],R[2]); } const p=P.pop(); const D=secWelzl(P,R); if(D&&isIn(D,p)) return D; return secWelzl(P,R.concat([p])); }
...
let INSEE_TYPO = {}; let CP_INDEX = {};
const PATH_TYPO='data/insee_typo_2025.json', PATH_CP='data/cp_communes_index.json', PATH_SDIS='data/sdis_sis.geojson';
async function loadLocalJSON(url){ try{ const r=await fetch(url,{cache:'no-store'}); if(!r.ok) throw new Error(`HTTP ${r.status}`); return await r.json(); }catch(e){ console.warn(`Chargement ${url} KO:`,e); return null; }}
...
async function fetchCommuneByPoint(lat,lon){
  ...
  try{ const r=await fetch(url,{cache:'no-store'}); if(!r.ok) throw new Error(`HTTP ${r.status}`); const a=await r.json(); return Array.isArray(a)&&a[0]?a[0]:null; }
  catch(e){ console.warn('Reverse commune KO',e); return null; }
}
function diameterFromTypology(ty){ const t=(ty||'').toLowerCase(); if(t.includes('urbain')) return 2; if(t.includes('péri')||t.includes('peri')) return 10; if(t.includes('rural')) return 20; return null; }
function setDiameterFromInsee(codeInsee, name){ const ty=INSEE_TYPO[codeInsee]||null; const D=diameterFromTypology(ty); if(D){ setDiameter(D); setStatus(`Commune: ${name||codeInsee} • Typologie: ${ty} • D=${D} km`); } else { setStatus(`Typologie inconnue pour ${name||codeInsee}. Choisis 2/10/20 km.`); } }
...
function setupMap(){
  ...
  app.map.on('click',e=>{
    if(app.mode==='producer'){ setProducer({lat:e.latlng.lat,lon:e.latlng.lng},{autoCommune:true}); setStatus('Producteur défini/déplacé'); return; }
    if(app.mode==='consumer'){ addParticipant({id:newId(),nom:`Consommateur ${app.participants.length+1}`,lat:e.latlng.lat,lon:e.latlng.lng,type:'consumer'}); setStatus('Consommateur ajouté'); return; }
  });
  ...
  app.layers.infoCtrl.onAdd=function(){ const d=L.DomUtil.create('div','acc-info-box'); d.style.cssText='margin:6px 0 0 6px;background:#0f1426cc;color:#fff;border:1px solid #20263d;padding:6px 8px;border-radius:8px;font-size:12px;line-height:1.3;user-select:none;min-width:210px'; d.innerHTML=infoBoxHTML({worstKm:null,ok:null}); L.DomEvent.disableClickPropagation(d); L.DomEvent.disableScrollPropagation(d); return d; };
...
function wireTabs(){
  const tabs=$('tabs'); const panels=document.querySelectorAll('.panel');
  if(!tabs) return;
  tabs.addEventListener('click',e=>{
    const b=e.target.closest('.tab'); if(!b) return;
    tabs.querySelectorAll('.tab').forEach(t=>{ t.classList.remove('active'); t.setAttribute('aria-pressed','false'); });
...
function refreshInfoBox(worstKm=null,ok=null){
  const boxes=document.getElementsByClassName('acc-info-box'); if(boxes&&boxes[0]) boxes[0].innerHTML=infoBoxHTML({worstKm,ok});
}
function enableLongPressAddOnMap(){
  let t=null,started=false,startPt=null;
  app.map.on('touchstart',e=>{
    started=true; const t0=e.touches?.[0]; if(!t0) return; startPt={x:t0.clientX,y:t0.clientY};
    t=setTimeout(()=>{ if(!started) return;
      ...
      addParticipant({id:newId(),nom,lat:pt.lat,lon:pt.lng,type:'consumer'}); setStatus('Consommateur ajouté (pression longue)');
    },650);
  },{passive:true});
...
function setProducer({lat,lon},opts={}){
  ...
  if(!app.layers.producer){
    app.layers.producer=L.marker([lat,lon],{
      draggable:true,title:'Producteur',
    ...
    app.layers.producer.on('dragend',()=>{ const {lat:la,lng:lo}=app.layers.producer.getLatLng(); setProducer({lat:la,lon:lo},{autoCommune:true}); });
    // long-press / dbl / contextmenu => supprimer
    let t=null,pressed=false;
    app.layers.producer.on('touchstart',()=>{ pressed=true; t=setTimeout(()=>{ if(!pressed) return; if(confirm('Supprimer le producteur ?')){ clearProducer(); setStatus('Producteur supprimé'); } },650); });
    app.layers.producer.on('touchend',()=>{ pressed=false; if(t) clearTimeout(t); });
    app.layers.producer.on('dblclick',()=>{ if(confirm('Supprimer le producteur ?')){ clearProducer(); setStatus('Producteur supprimé'); } });
    app.layers.producer.on('contextmenu',()=>{ if(confirm('Supprimer le producteur ?')){ clearProducer(); setStatus('Producteur supprimé'); } });
  } else {
    app.layers.producer.setLatLng([lat,lon]);
  }
  afterModelChange();
  if(opts.autoCommune){
    (async()=>{ const c=await fetchCommuneByPoint(lat,lon); if(c?.code){ setDiameterFromInsee(c.code,c.nom); } })();
  }
}
function clearProducer(){ app.producteur=null; if(app.layers.producer){ app.map.removeLayer(app.layers.producer); app.layers.producer=null; } afterModelChange(); }

function bindMarkerUI(marker,p){
  const secC=app.secCenter; const dToProd=app.producteur?distKm(p,app.producteur):null; const dToC=secC?distKm(p,secC):null;
  const html=`<div style="min-width:190px"><b>${p.nom||'Participant'}</b><br>${p.type||'consumer'}<br>${p.lat.toFixed(5)}, ${p.lon.toFixed(5)}<br>
  ...
  <div style="display:flex;gap:6px;margin-top:6px"><button id="center-${p.id}" class="btn" style="flex:1">Centrer</button><button id="del-${p.id}" class="btn danger" style="flex:1">Supprimer</button></div></div>`;
  marker.bindPopup(html);
  marker.on('popupopen',()=>{
    const bDel=$(`del-${p.id}`), bCtr=$(`center-${p.id}`);
  ...
  let t=null,pressed=false;
  marker.on('touchstart',()=>{ pressed=true; t=setTimeout(()=>{ if(!pressed) return; if(confirm(`Supprimer "${p.nom}" ?`)){ removeParticipant(p.id); setStatus('Participant supprimé'); } },650); });
  marker.on('touchend',()=>{ pressed=false; if(t) clearTimeout(t); });
}
function addParticipant(p){ if(!isValidLatLon(p.lat,p.lon)) return showError('Coordonnées participant invalides'); app.participants.push(p); afterModelChange(); }
...
function redrawParticipants(){
  ...
  app.participants.forEach(p=>{ const color=p.type==='sdis'?'#ff3b30':(p.type==='producer'?'#f5b841':'#4ea2ff'); const m=L.circleMarker([p.lat,p.lon],{radius:6,color,weight:2,fillOpacity:.8}).addTo(app.layers.parts); bindMarkerUI(m,p); });
}
...
function drawWorstOverlay(w,D){
  clearWorstOverlay(); if(!w?.a||!w?.b) return; const ok=w.d<=D;
...
function drawAccCircle(centerLike,ok){
  const c=toLatLon(centerLike); if(!c) return; const rMeters=(app.distMaxKm/2)*1000; const color=ok?'#2ecc71':'#e67e22';
  if(!app.layers.accCircle){ app.layers.accCircle=L.circle([c.lat,c.lon],{radius:rMeters,color,weight:3,opacity:1,fillOpacity:.06,fillColor:color,dashArray:'8,6'}).addTo(app.map); }
  else{ app.layers.accCircle.setLatLng([c.lat,c.lon]); app.layers.accCircle.setRadius(rMeters); app.layers.accCircle.setStyle({color,fillColor:color}); }
}
function getAllPoints(){ const pts=[]; if(app.producteur) pts.push({...app.producteur,type:'producer'}); app.participants.forEach(p=>pts.push(p)); return pts; }
function updateCompliance(){
  const pts=getAllPoints(); const sec=smallestEnclosingCircle(pts); app.secCenter=sec?.c||(app.producteur||null); app.secRadiusKm=sec?.rKm||0;
  let worst=null, ok=null; if(pts.length>=2){ worst=worstPair(pts); ok=worst.d<=app.distMaxKm; drawWorstOverlay(worst,app.distMaxKm);} else clearWorstOverlay();
  const mapCenterLeaf=(app.map&&typeof app.map.getCenter==='function')?app.map.getCenter():{lat:45.191,lng:5.684}; const mapCenter=toLatLon(mapCenterLeaf)||{lat:45.191,lon:5.684};
  const fallbackCenter=app.producteur?app.producteur:mapCenter; const centerLike=app.secCenter||fallbackCenter;
  const okBySEC=(app.secRadiusKm*2)<=app.distMaxKm; drawAccCircle(centerLike,okBySEC);
...
function fitToProject(){
  ...
  if(app.layers.accCircle && typeof app.layers.accCircle.getBounds==='function'){ bounds.extend(app.layers.accCircle.getBounds()); }
  if(app.layers.producer && typeof app.layers.producer.getLatLng==='function'){ bounds.extend(app.layers.producer.getLatLng()); }
  if(app.layers.parts && typeof app.layers.parts.eachLayer==='function'){ app.layers.parts.eachLayer(l=>{ if(typeof l.getBounds==='function') bounds.extend(l.getBounds()); else if(typeof l.getLatLng==='function') bounds.extend(l.getLatLng()); }); }
  if(app.layers.worstLine && typeof app.layers.worstLine.getBounds==='function'){ bounds.extend(app.layers.worstLine.getBounds()); }
  if(app.layers.worstLabel && typeof app.layers.worstLabel.getLatLng==='function'){ bounds.extend(app.layers.worstLabel.getLatLng()); }
  if(!bounds.isValid()) return; app.map.fitBounds(bounds.pad(0.25),{maxZoom:15});
}

/* ===== CP -> communes (jeu local) ===== */
function wirePostalLookup(){
  const input=$('cpInput'), btn=$('btnSearchCP'), list=$('cpResults'); if(!input||!btn||!list) return;
  btn.addEventListener('click',()=>{
    const cp=(input.value||'').trim(); if(!/^\d{5}$/.test(cp)) return showError('Code postal à 5 chiffres requis');
    const communes=CP_INDEX[cp]||[]; if(communes.length===0){ list.innerHTML='<li style="opacity:.7">Aucune commune (jeu local)</li>'; return; }
    list.innerHTML=''; communes.forEach(c=>{ const li=document.createElement('li'); li.textContent=`${c.nom} — INSEE ${c.code}`; li.onclick=()=>{ if(isFiniteNum(c.lat)&&isFiniteNum(c.lon)) app.map.setView([c.lat,c.lon], Math.max(12, app.map.getZoom())); setDiameterFromInsee(c.code,c.nom); }; list.appendChild(li); });
  });
}
...
async function ensureSdisLayerLoaded(){
  ...
  const gj=await loadLocalJSON(PATH_SDIS); if(!gj){ showError('SDIS/SIS indisponibles (data/sdis_sis.geojson manquant)'); return false; }
  app.layers.sdisLayer=L.geoJSON(gj,{
    ...
    onEachFeature:(feat,layer)=>{
      const props=feat.properties||{}; const name=props.nom||props.NOM||'SDIS/SIS'; let ll=null;
      if(typeof layer.getLatLng==='function'){ const raw=layer.getLatLng(); ll={lat:Number(raw.lat),lon:Number(raw.lng)}; }
      if(!ll && feat.geometry && feat.geometry.type==='Point' && Array.isArray(feat.geometry.coordinates)){ const [gx,gy]=feat.geometry.coordinates; if(isFiniteNum(gx)&&isFiniteNum(gy)) ll={lat:gy,lon:gx}; }
      if(ll) layer.bindPopup(`<b>${name}</b><br>${ll.lat.toFixed(5)}, ${ll.lon.toFixed(5)}`); else layer.bindPopup(`<b>${name}</b>`);
      layer.on('dblclick',()=>{ if(!ll) return; addParticipant({id:newId(),nom:name,lat:ll.lat,lon:ll.lon,type:'sdis'}); setDiameter(20); setStatus('SDIS ajouté (diamètre = 20 km)'); });
    }
  });
  return true;
}
...
function setDiameter(d){
  ...
  onProjectChanged(); setStatus(`Diamètre = ${D} km`);
}
function wireDiameterChips(){
  const wrap=$('chipDiameter'); if(!wrap) return;
  wrap.addEventListener('click',e=>{ const btn=e.target.closest('.chip[data-d]'); if(!btn) return; setDiameter(btn.getAttribute('data-d')); });
...
/* ===== Reactions ===== */
function afterModelChange(){ redrawParticipants(); updateCompliance(); saveProject(); }
function onProjectChanged(){ updateCompliance(); saveProject(); }
